...
exports.timedhash = function(bot) {
  ...
  function sendMiningInfo(bot) {
    let embed
    needle.get('http://104.238.153.140:3001/api/getblockcount', function(
      error,
      response
    ) {
      if (error || response.statusCode !== 200) {
        ...
        needle.get('http://104.238.153.140:3001/api/getnetworkhashps', function(
          error,
          response
        ) {
          let hashrate = Number(response.body)
          needle.get('http://104.238.153.140:3001/api/getdifficulty', function(
            error,
            response
          ) {
    ...
    function sendMiningInfo(bot, msg, suffix) {
      ...
      needle.get('http://104.238.153.140:3001/api/getblockcount', function(
        ...
        if (error || response.statusCode !== 200) {
          ...
          let height = Number(response.body)
          needle.get(
            'http://104.238.153.140:3001/api/getnetworkhashps',
            function(error, response) {
              let hashrate = Number(response.body)
              needle.get(
                'http://104.238.153.140:3001/api/getdifficulty',
                function(error, response) {
                  let difficulty = Number(response.body)
                  embed = {
                    color: 1741945,
    ...
    function sendProfitInfo(bot, msg, suffix) {
      ...
      needle.get('http://104.238.153.140:3001/api/getnetworkhashps', function(
        ...
        if (error || response.statusCode !== 200) {
          ...
          let rewardperblock = Number(5) // block reward
          ...
          let totalrewardperhour =
            60 * 60 / blocktime * rewardperblock * percentage

          let PEXA = totalrewardperhour
          ...
          needle.get(
            'https://api.coinmarketcap.com/v1/ticker/Pexa Coin/',
            function(error, response) {
              let author1 =
                'With ' + numberWithCommas(parseFloat(myhashrate)) + ' Mh/s'
              let name1 =
                'You can potentially earn the following amounts of **PEXA**'
              let val1 =
                '```1 Hour  = ' +
                PEXA.toFixed(8) +
                '\n' +
              ...
              let val2 =
                '```1 Hour  = ' +
                (PEXA * Number(response.body[0].price_btc)).toFixed(8) +
                '\n' +
                '1 Day   = ' +
                (CHC24 * Number(response.body[0].price_btc)).toFixed(8) +
                '\n' +
                '1 Week  = ' +
                (CHC1w * Number(response.body[0].price_btc)).toFixed(8) +
                '\n' +
                '1 Month = ' +
                (CHC1m * Number(response.body[0].price_btc)).toFixed(8) +
                ' \n' +
                '1 Year  = ' +
                (CHC1y * Number(response.body[0].price_btc)).toFixed(8) +
                '```'
              ...
              let val3 =
                '```1 Hour  = ' +
                numberWithCommas(
                  (PEXA * Number(response.body[0].price_usd)).toFixed(2)
                ) +
                '\n' +
                '1 Day   = ' +
                numberWithCommas(
                  (CHC24 * Number(response.body[0].price_usd)).toFixed(2)
                ) +
                '\n' +
                '1 Week  = ' +
                numberWithCommas(
                  (CHC1w * Number(response.body[0].price_usd)).toFixed(2)
                ) +
                '\n' +
                '1 Month = ' +
                numberWithCommas(
                  (CHC1m * Number(response.body[0].price_usd)).toFixed(2)
                ) +
                ' \n' +
                '1 Year  = ' +
                numberWithCommas(
                  (CHC1y * Number(response.body[0].price_usd)).toFixed(2)
                ) +
                '```'
