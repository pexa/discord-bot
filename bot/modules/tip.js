...
const pexaConfig = config.get('pexacore')
const pexaDaemon = new Client(pexaConfig)
let ChannelID = config.get('Channels').rainroom
...
exports.log = function(bot) {
  ...
  async function log(bot) {
    let embed
    let servicesfee = await pexaDaemon.getBalance('services fee')
    let bet = await pexaDaemon.getBalance('bet')
    let unclaimed = parseFloat(await pexaDaemon.getBalance('unclaimed'))
    let added = parseFloat(0)
    let current = unclaimed
    let accountlist = Object.keys(await pexaDaemon.listAccounts())
    ...
    for (var i = 0; i < accountlist.length; i++) {
      let balance = parseFloat(await pexaDaemon.getBalance(accountlist[i]))
      added = added + balance
      if (balance != 0.0) {
        let result = await pexaDaemon.move(
          accountlist[i],
          'unclaimed',
          balance.toFixed(8),
          1,
  ...
  process: async function(bot, msg, suffix) {
    if (!inPrivate(msg)) {
      const embed = {
        ...
        description:
          'Generated ' + symbol + ' deposit address sent as private message.',
      }
      msg.channel.send({
        embed,
      })
    ...
    let address = await pexaDaemon.getAddressesByAccount(msg.author.id)

    if (address[0] == null) {
      pexaDaemon.getAccountAddress(msg.author.id).then((result, error) => {
        if (error) {
          console.log(error)
          return
        } else {
...
exports.balance = {
  ...
  process: function(bot, msg, suffix) {
    pexaDaemon.getBalance(msg.author.id, 1).then((result, error) => {
      if (error) {
        console.log(error)
        return
      } else {
...
exports.withdraw = {
  ...
  description:
    'Withdraw ' + symbol + ', 0.1 ' + symbol + ' fee for withdrawing.',
  process: function(bot, msg, suffix) {
    ...
    if (words.length != 3) {
      const embed = {
        ...
        description:
          'Sorry, Please use **!withdraw [amount] [address] or !withdraw [address] [amount]**',
      }
      msg.author.send({
        embed,
      })
    ...
    if (
      !Number.isNaN(parseFloat(words[2])) ||
      words[2].toUpperCase() == 'all'.toUpperCase()
    ) {
      let temp = words[2]
      words[2] = words[1]
      words[1] = temp
    ...
    if (
      (Number.isNaN(parseFloat(words[1])) &&
        words[1].toUpperCase() != 'all'.toUpperCase()) ||
      (getValidatedAmount(words[1]) == null &&
        words[1].toUpperCase() != 'all'.toUpperCase()) ||
    ...
    pexaDaemon.getBalance(msg.author.id, 1).then((before, error) => {
      ...
      if (parseFloat(words[1]) <= parseFloat(before) - 0.1) {
        ...
        const embed = {
          ...
          description:
            'Please check your ' + symbol + ' with **!balance** command.',
        }
        msg.author.send({
          embed,
        })
    ...
    if (words.length != 3) {
      const embed = {
        ...
        description:
          '<@' +
          msg.author.id +
          '> Please use **!tip [amount] [@username] or !tip [@username] [amount]**',
      }
      msg.channel.send({
        embed,
      })
    ...
    if (
      ...
      const embed = {
        ...
        description:
          '<@' +
          msg.author.id +
          '> Please **!tip [amount] [@username] or !tip [@username] [amount]**',
      }
      msg.channel.send({
        embed,
      })
        ...
        if (msg.author.id == msg.mentions.members.first().id) {
          const embed = {
            ...
            description:
              '<@' +
              msg.author.id +
              "> Sorry, you're not allowed to tip yourself.",
          }
          msg.channel.send({
            embed,
          })
...
exports.soak = {
  ...
  process: async function(bot, msg, suffix) {
    ...
    let balance = await pexaDaemon.getBalance(msg.author.id)
    if (words[1].toUpperCase() == 'all'.toUpperCase()) {
      words[1] = balance
    }
    ...
    if (parseFloat(words[1]) < 1) {
      const embed = {
        ...
        description:
          '<@' + msg.author.id + '> Minimum amount for soak is **1** ' + symbol,
      }
      msg.channel.send({
        embed,
      })
    ...
    let header =
      '<@' + msg.author.id + '> Sent **' + soak + '** ' + symbol + ' to:'
    for (var i = 0; i < list.length; i++) {
      let moveresult = await pexaDaemon.move(
        msg.author.id,
        list[i].id,
        soak,
        1,
  ...
  process: async function(bot, msg, suffix, isEdit, myCache) {
    ...
    if (parseFloat(words[1]) < 1) {
      const embed = {
        ...
        description:
          '<@' + msg.author.id + '> Minimum amount for rain is **1** ' + symbol,
      }
      msg.channel.send({
        embed,
      })
    ...
    let header =
      '<@' + msg.author.id + '> Sent **' + rain + '** ' + symbol + ' to:'
    arr = new Array()
  ...
  process: async function(bot, msg, suffix) {
    ...
    if (words.length == 1) {
      const embed = {
        ...
        description:
          '<@' + msg.author.id + '> Please use **!megasoak [amount]**',
      }
      msg.channel.send({
        embed,
      })
    ...
    if (parseFloat(words[1]) < 1) {
      const embed = {
        ...
        description:
          '<@' +
          msg.author.id +
          '> Minimum amount for megasoak is **1** ' +
          symbol,
      }
      msg.channel.send({
        embed,
    ...
    if (moveresult == 'false') {
      ...
      balance = await pexaDaemon.getBalance('megasoak')
      const embed = {
        color: 1741945,
        timestamp: new Date(),
        footer: {
    ...
    let header =
      'Thank for active, you receive **' + soak + '** ' + symbol + ':'
    for (var i = 0; i < list.length; i++) {
      let moveresult = await pexaDaemon.move(
        'megasoak',
        list[i].id,
    ...
    if (parseFloat(words[1]) < 1 || parseFloat(words[1]) > 2000) {
      const embed = {
        ...
        description:
          '<@' +
          msg.author.id +
          '> Bet amount can not > **2000** or can not < **1** ' +
          symbol,
      }
      msg.channel.send({
        embed,
...
function getValidatedAmount(amount) {
  amount = amount.trim()
  if (amount.toLowerCase().endsWith('pexaDaemon')) {
